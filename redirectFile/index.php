...
$randomUsers = file_get_contents("https://randomuser.me/api/?results=" . $config['count']."&gender=". $config['gender']);
...
$configJson = $config['share_icons'];
$configJson = str_replace(array('{','}'),'',json_encode($configJson));
...
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.bundle.min.js"
        ...
        crossorigin="anonymous"></script>
<script>
    new mShare('.demo', {size:'small',"url":"<?php echo $config['share'] ?>", <?php echo $configJson ?>});
    var jsonData = "";
    ...
    $(".shareBtns a").on('click', function (e) {
        ...
        let type  = lastClass.split('mshare-item--').pop();
        let url = $(this).attr('href');
        let id = $(this).closest('.shareBtns').data('id');
        openPopup(url, id,500,500,type);
    });

    function openPopup(url, id, width, height,type) {
        url = jsonData.share_urls[type] +  jsonData.share;
